...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=self.model_embeddings.embed_size, hidden_size=self.hidden_size, bidirectional=True)
        self.decoder = nn.LSTMCell(input_size=self.model_embeddings.embed_size + self.hidden_size , hidden_size=self.hidden_size)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, sequence_lengths = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for yt in torch.split(Y, split_size_or_sections=1, dim=0):
            yt = torch.squeeze(yt, 0)
            ybart = torch.cat((yt, o_prev), 1)
            dec_state, ot, et = self.step(Ybar_t=ybart, dec_state=dec_state, enc_hiddens=enc_hiddens, enc_hiddens_proj=enc_hidden_proj, enc_masks=enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        O_t = self.dropout(torch.Tensor.tanh(V_t))
        combined_output = O_t
